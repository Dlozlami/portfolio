...
export default function CV() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.storage.from('cv').list('', { limit: 1, sortBy: { column: 'created_at', order: 'desc' } }),
    ]).then(([p, w, s, c, cv]) => {
      setProfile(p.data)
      ...
      if (cv.data && cv.data.length > 0) {
        const { data } = supabase.storage.from('cv').getPublicUrl(cv.data[0].name)
            ...
            {certifications.map(cert => (
              <div key={cert.id} className={styles.certRow}>
                ...
                <div>
                  ...
                  <div className={styles.certIssuer}>{cert.issuer}{cert.year ? ` · ${cert.year}` : ''}</div>
                </div>
              </div>
            ))}
          </section>
